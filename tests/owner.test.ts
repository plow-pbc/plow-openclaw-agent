...
for (const kind of ["group", "direct", "email"]) for (const role of ["owner", "member"]) for (const trusted of [false, true]) test(`roster identity preserves routing without restricting tools: ${kind}, ${role}, trusted=${trusted}`, async t => {
  ...
  const sender = { type: "member", uid: "local-sender", role, display_name: role === "owner" || kind === "group" ? "+15550000001" : "Sender", provider_key: "+15550000001" };
  ...
  const logs: string[] = [];
  let context: { access?: { toolPolicy?: { deny: string[] } }; from: string; sender: { id: string; name: string }; conversation: { id: string; routePeer: Peer }; message: { rawBody: string }; supplemental: { channelStructuredContext: { payload: { trusted: boolean; participants: { role: string; name: string }[] } }[] } } | undefined;
  ...
  assert.ok(channel);
  await channel.gateway.startAccount({ account, cfg: { commands: { ownerAllowFrom: ["plow-owner"] } }, abortSignal: controller.signal, log: { info(text: string) { logs.push(text); } } });
  assert.ok(context);
  if (role === "owner") assert.equal(context.sender.id, "plow-owner");
  else assert.match(context.sender.id, /^plow-person:[a-f0-9]{64}$/);
  if (role === "owner") {
    assert.equal(context.sender.name, "owner");
    assert.equal(context.supplemental.channelStructuredContext[0].payload.participants[0].name, "owner");
    assert.ok(logs.some(line => line.includes('"senderName":"owner"')));
  } else if (kind === "group") {
    assert.equal(context.sender.name, "unnamed member");
    assert.ok(logs.some(line => line.includes('"senderName":"member …0001"')));
    assert.ok(!JSON.stringify(context).includes("…0001"));
  } else {
    assert.equal(context.sender.name, "Sender");
  }
...
test("one member keeps a private person id across group seats", async t => {
  ...
  const noHandle = { type: "member", uid: "cp_nokey", role: "member", display_name: "" };
  const chats = [
    { uid: "group-one", status: "active", trusted: true, participants: [member("cp_one", handle), agent, member("cp_else", otherHandle)] },
    { uid: "group-two", status: "active", trusted: true, participants: [member("cp_two", handle.toUpperCase()), agent, member("cp_other", otherHandle), noHandle] },
  ];
  t.mock.method(globalThis, "fetch", async (url: string) => Response.json(
  ...
  server.on("connection", (socket: { send: (text: string) => void }) => {
    for (const [i, chat, sender] of [[0, chats[0], chats[0].participants[0]], [1, chats[1], chats[1].participants[0]], [2, chats[1], chats[1].participants[2]], [3, chats[1], noHandle]] as const) {
  ...
  entry.register({ registrationMode: "full", registerTool() {}, logger: { info() {} },
    ...
    runtime: { channel: {
      ...
      inbound: { buildContext: async (value: typeof contexts[number]) => { contexts.push(value); return {}; },
        dispatch: async () => { if (contexts.length === 4) controller.abort(); return { dispatched: true, dispatchResult: { deliberateSilentTerminalReply: true } }; } },
    } },
  });
  assert.ok(channel);
  ...
  assert.match(byMessage.get("inbound-3")!.sender.id, /^plow-person:[a-f0-9]{64}$/);
  assert.equal(byMessage.get("inbound-3")!.sender.name, "unnamed member");
  assert.ok(logs.some(line => line.includes('"senderName":"member p…n@example.test"')));
